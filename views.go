...
//
// GENERATOR-GAPS TODO (tranche P6a): the container constructors in this
// file (VStack, HStack, ZStack, List, Form, TabView, LazyV/HStack, and the
// aligned/spaced variants) were hand-edited to route their child-pointer
// marshalling through withContainerScratch (see view_builder.go) instead of
// an inline `ptrs := make([]uintptr, len(children))`. When this file is
// regenerated, the template driving these container stubs must be updated
// to emit the withContainerScratch pattern, otherwise the P6a view-builder
// arena will silently regress to per-call slice allocation.
...
// PasteButtonPayload creates a clipboard button that reports a concrete paste payload.
func PasteButtonPayload(label string, action func(PastePayload) bool) View {
	if action == nil {
		return Button(label, func() {})
	}
	actionID := registerPasteCallback(action)
	var ptr uintptr
	withCString(label, func(labelC *byte) {
		ptr = _SUIPasteButtonPayload(labelC, actionID)
	})
	ret := View{ptr: ptr, retained: newRetainedOwned(ptr)}
	ret.retained.addCallbackID(actionID)
	return ret
