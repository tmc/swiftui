...
public func SUIViewTextFieldStyle(_ viewRef: UnsafeMutableRawPointer, _ style: Int32) -> UnsafeMutableRawPointer {
    let base = Unmanaged<Box<AnyView>>.fromOpaque(viewRef).takeUnretainedValue().value
    let configured = base.environment(\.suiTextFieldStyleKind, style)
    let view: AnyView
    switch style {
    case 0: view = AnyView(configured.textFieldStyle(.automatic))
    case 1: view = AnyView(configured.textFieldStyle(.roundedBorder))
    case 2: view = AnyView(configured.textFieldStyle(.plain))
    default: view = AnyView(configured.textFieldStyle(.automatic))
    }
    return Unmanaged.passRetained(Box(view)).toOpaque()
}
...
public func SUIViewScrollContentBackgroundHidden(_ viewRef: UnsafeMutableRawPointer) -> UnsafeMutableRawPointer {
    let base = Unmanaged<Box<AnyView>>.fromOpaque(viewRef).takeUnretainedValue().value
    let view = AnyView(base.environment(\.suiScrollContentBackgroundHidden, true).scrollContentBackground(.hidden))
    return Unmanaged.passRetained(Box(view)).toOpaque()
}
...
@_cdecl("SUIViewAnimationCurve")
public func SUIViewAnimationCurve(_ viewRef: UnsafeMutableRawPointer,
                                  _ kind: Int32,
                                  _ duration: Double) -> UnsafeMutableRawPointer {
    let base = Unmanaged<Box<AnyView>>.fromOpaque(viewRef).takeUnretainedValue().value
    let curve: Animation
    switch kind {
    case 0: curve = duration > 0 ? .easeInOut(duration: duration) : .easeInOut
    case 1: curve = duration > 0 ? .easeIn(duration: duration)    : .easeIn
    case 2: curve = duration > 0 ? .easeOut(duration: duration)   : .easeOut
    case 3: curve = .spring
    case 4: curve = .bouncy
    default: curve = .default
    }
    let view = AnyView(base.animation(curve, value: true))
    return Unmanaged.passRetained(Box(view)).toOpaque()
}
...
public func SUIViewScaleEffectAnchor(_ viewRef: UnsafeMutableRawPointer,
                                     _ scale: Double,
                                     _ anchor: Int32) -> UnsafeMutableRawPointer {
    let base = Unmanaged<Box<AnyView>>.fromOpaque(viewRef).takeUnretainedValue().value
    let point: UnitPoint
    switch anchor {
    case 1: point = .top
    case 2: point = .bottom
    case 3: point = .leading
    case 4: point = .trailing
    case 5: point = .topLeading
    case 6: point = .topTrailing
    case 7: point = .bottomLeading
    case 8: point = .bottomTrailing
    default: point = .center
    }
    let view = AnyView(base.scaleEffect(scale, anchor: point))
    return Unmanaged.passRetained(Box(view)).toOpaque()
}
...
public func SUIViewOnTapGestureCount(_ viewRef: UnsafeMutableRawPointer,
                                     _ count: Int32,
                                     _ callbackID: UInt) -> UnsafeMutableRawPointer {
    let base = Unmanaged<Box<AnyView>>.fromOpaque(viewRef).takeUnretainedValue().value
    let id = callbackID
    let view = AnyView(base.onTapGesture(count: Int(count)) {
        _SUIButtonCallback?(id)
    })
    return Unmanaged.passRetained(Box(view)).toOpaque()
}
...
// SUIModifierReader now lives in bridge_helpers.swift so that both
// bridge_modifiers.swift (packed modifier chains) and bridge_views.swift
// (Path/Canvas opcode decoder) can share one little-endian byte reader.
